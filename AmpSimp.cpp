...
void biquad_process(Biquad *f, float *buffer, u32 n_samples) {

    for (u32 index = 0; index < n_samples; index++) {
...
void shelf_process(ShelfFilter *f, float *buffer, u32 n_samples) {
...
struct DSPData {
    ...
    struct NoiseGate {
        ...
        float attack_time_ms = 1.0;
        float release_time_ms = 15.0;
        float hysteresis = 0.0;
        float return_gain = 0.0;
        bool isOpen = false;
...
void grid_conduction(float *buffer, u32 n_samples) {
...
void tube_sim(float *buffer, u32 n_samples, float pre_bias, float post_bias) {
...
{   
    ...
    // preamp
    {
        ...
        memset(up_buffer, 0, UP_BLOCK_SIZE * sizeof(float));
        for (u32 index = 0; index < BLOCK_SIZE; index++) {
            up_buffer[UPSAMPLE_FACTOR * index] = audio_buffer[index]; 
        }
        
        biquad_process(&preamp.oversampler.upsample_filter1, up_buffer, UP_BLOCK_SIZE);
        biquad_process(&preamp.oversampler.upsample_filter2, up_buffer, UP_BLOCK_SIZE);
        apply_gain_linear(UPSAMPLE_FACTOR, up_buffer, UP_BLOCK_SIZE);
    
        {
            // ------------ Stage 0 ------------
            grid_conduction(up_buffer, UP_BLOCK_SIZE);
            shelf_process(&preamp.cathode_bypass_filter0, up_buffer, UP_BLOCK_SIZE);
            tube_sim(up_buffer, UP_BLOCK_SIZE, preamp.stage0_bias[0], preamp.stage0_bias[1]);
            
            preamp.input_filter.ProcessBlock(up_buffer, UP_BLOCK_SIZE);
            preamp.stage0LP.ProcessBlock(up_buffer, UP_BLOCK_SIZE);
            
            apply_gain_linear(dsp.param_states.gain1, up_buffer, UP_BLOCK_SIZE);
            ...
            // ------------ Stage 1 ------------
            grid_conduction(up_buffer, UP_BLOCK_SIZE);
            shelf_process(&preamp.cathode_bypass_filter1, up_buffer, UP_BLOCK_SIZE);
            tube_sim(up_buffer, UP_BLOCK_SIZE, preamp.stage1_bias[0], preamp.stage1_bias[1]);
            
            preamp.couplingFilter1.ProcessBlock(up_buffer, UP_BLOCK_SIZE);
            preamp.stage1LP.ProcessBlock(up_buffer, UP_BLOCK_SIZE);
            
            apply_gain_linear(dsp.param_states.gain2, up_buffer, UP_BLOCK_SIZE);


            // ------------ Stage 2 ------------
            grid_conduction(up_buffer, UP_BLOCK_SIZE);
            shelf_process(&preamp.cathode_bypass_filter2, up_buffer, UP_BLOCK_SIZE);
            tube_sim(up_buffer, UP_BLOCK_SIZE, preamp.stage2_bias[0], preamp.stage2_bias[1]);
            ...
            apply_gain_linear(preamp.attenuation2, up_buffer, UP_BLOCK_SIZE);
            
            
            // ------------ Stage 3 ------------
            grid_conduction(up_buffer, UP_BLOCK_SIZE);
            shelf_process(&preamp.cathode_bypass_filter3, up_buffer, UP_BLOCK_SIZE);
            tube_sim(up_buffer, UP_BLOCK_SIZE, preamp.stage3_bias[0], preamp.stage3_bias[1]);
            
            preamp.couplingFilter3.ProcessBlock(up_buffer, UP_BLOCK_SIZE);
            preamp.stage3LP.ProcessBlock(up_buffer, UP_BLOCK_SIZE);
    
    
            apply_gain_linear(preamp.attenuation3, up_buffer, UP_BLOCK_SIZE);
    
    
            // ------------ Stage 4 ------------
            grid_conduction(up_buffer, UP_BLOCK_SIZE);
            shelf_process(&preamp.cathode_bypass_filter4, up_buffer, UP_BLOCK_SIZE);
            tube_sim(up_buffer, UP_BLOCK_SIZE, preamp.stage4_bias[0], preamp.stage4_bias[1]);
        ...
        biquad_process(&preamp.oversampler.downsample_filter1, up_buffer, UP_BLOCK_SIZE);
        biquad_process(&preamp.oversampler.downsample_filter2, up_buffer, UP_BLOCK_SIZE);
...
int main(void) {
    ...
    dsp.upsamplerate = 4.0f * samplerate;
...

    {
        ...
        gate.buffer_length = (u32)(samplerate * gate_buffer_length_sec);
        gate.buffer_index = 0;
        gate.absolute_sum = 0.0;
        gate.threshold = dbtoa(-75.0);
        gate.return_gain = gate.threshold;
        gate.isOpen = false;

        gate.buffer = (float *)calloc(gate.buffer_length, sizeof(float));        
        smooth_param_init(&gate.gain, 0.0);
    }
...

    {
        ...
        preamp.input_filter.SetFrequency(300.0f/dsp.upsamplerate); // 100.0
        ...
        biquad_set_coeffs(&preamp.oversampler.upsample_filter2, dsp.samplerate/2.0f * 0.9f, 
                            1.3065630, 0.0f, dsp.upsamplerate, BIQUAD_LOWPASS);
